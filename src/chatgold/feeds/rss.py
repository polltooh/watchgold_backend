    ...
    for url in urls:
        try:
            ...
            for item in items:
                ...
                clean_desc = BeautifulSoup(description, "html.parser").get_text(separator=" ", strip=True) if description else ""
...
def summarize_with_ai(articles: list[dict]) -> list[dict]:
    ...
    articles_text = ""
    valid_articles = [art for art in articles if art.get('link') and art['link'].strip() and art['link'] != "No Link"]
    for idx, art in enumerate(valid_articles[:100]): # Limit to first 100 to avoid token limits
...
def get_daily_summary() -> list[dict]:
    ...
    downloads_dir = os.path.join(os.path.dirname(__file__), "..", "crawler", "downloads")
    ...
    if isinstance(top_summary, list):
        ...
        for item in top_summary:
            item["detailed_markdown"] = generate_detailed_markdown(item, chat_model)
